...
Built (offline core, `packages/asset-pipeline/`, tested but unproven in a lab): versioned style bible + asset manifest/ledger schemas, deterministic post-processing (nearest-neighbor scale, palette quantization, alpha snap), check suite, provider protocol with a local-file provider, bounded regenerate-with-feedback runner, a stubbed vision-judge interface for the visual-eval leg to fill, and the `Studio` facade that seals all of this behind the public boundary. Also built **and now exercised live** (2026-07-19): the `gpt-image-1.5` generalist (one real sprite through generate→checks, all deterministic checks passed) and the **local SDXL + Pixel Art XL specialist** (`local_sd.py` — ComfyUI HTTP client + a `rembg` background-removal node that supplies the alpha SDXL lacks; one real sprite passed all checks with a clean transparent cutout), plus **per-class routing** (`Studio.routed(...)` → an internal `CategoryRouter`: sprites→`local_sd`, everything else→`gpt-image`). Open: **sprite-scale quality tuning** — at 32×32 through the generic-palette pipeline the specialist output is a dark, silhouette-y read, so the "crisper than a downscale" advantage isn't yet visually confirmed at sprite scale (a human taste call + tuning of palette/target-size/LoRA-strength); real vision QA; audio; lab proof. In-Godot `.import` verification is **done** (2026-07-19): Godot 4.6.3 `--headless --import` accepted the generated sidecars (our values preserved, defaults filled in) and both smoke sprites load as `CompressedTexture2D` at correct dimensions.
